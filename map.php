...
if (isset($_GET['cropfinder'])) {
    ...
    function load()
        ...
        $this->warsAllianceId = array();
        if (0 < intval($this->data['alliance_id']))
            {
            $cont = trim($m->getwarAllianceId($this->data['alliance_id']));
            if ($cont != '')
                {
                $_arr = explode(',', $cont);
                foreach ($_arr as $warAllianceId)
                    {
                    $this->warAllianceId[$warAllianceId] = 1;
                    }
                }
            }
        ...
        if ($this->isPost())
            {
if ( intval($_POST['mxp']) < 401 && intval($_POST['mxp']) >'-401' )
        {
            $_x = intval($_POST['mxp']);
        }
        if ( intval($_POST['myp']) < 401 && intval($_POST['myp']) >'-401' )
        {
            $_y = intval($_POST['myp']);
        }
            }
        ...
        while ($result->next())
            ...
            $this->matrixSet[$result->row['id']] = array(
                ...
                'is_artefacts' => $result->row['is_artefacts'],
    ...
    function isContractWith($allianceId)
        {
        return (isset($this->contractsAllianceId[$allianceId]));
        }

    function isContractwarWith($allianceId)
        {
        return (isset($this->warAllianceId[$allianceId]));
        }
    ...
    function getCssClassName2($index)
        {
        return $this->getCssClassAtttt($this->matrixSet[$index]);
        }
    function getCssClassAtttt($vid)
        {
        }
    function getCssClassAttt($vid)
        {
        }
    function getCssClassNameByItem($mapItem)
        ...
        if ($mapItem['is_artefacts'] && $mapItem['player_name'] == 'التتار')
            {
            return 'o99';
            }
        if ($mapItem['player_id'] != '')
            ...
            if ($this->player->playerId == $mapItem['player_id'])
                ...
                if ($mapItem['alliance_id'])
                    ...
                    else if ($this->isContractwarWith($mapItem['alliance_id']))
                       {
                        $c2 = 2;
                       }
                    }
