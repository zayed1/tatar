...
define( "__QS_LOCK_FS_", APP_PATH."ls-f/smartservs-ls-v7" );
class MutexModel extends ModelBase
{

    public function lock( )
    {
        if ( 0 < $this->provider->executeQuery2( "UPDATE g_settings gs SET gs.qlocked=0, qlocked_date=NOW() WHERE gs.qlocked=0" ) && ( $fp = fopen( __QS_LOCK_FS_, "r" ) ) != FALSE )
        {
            if ( flock( $fp, LOCK_EX ) )
            {
                fclose( $fp );
