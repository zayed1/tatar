...
class ResourcesModel extends ModelBase
{
    ...
    {
        return $this->provider->fetchRow( "SELECT v.id, v.player_id, v.is_oasis, v.player_name, v.village_name, v.resources, v.cp, v.crop_consumption, TIME_TO_SEC(TIMEDIFF(NOW(), v.last_update_date)) elapsedTimeInSeconds FROM p_villages v WHERE v.id = %s", array(
            $id
        ) );
    }
