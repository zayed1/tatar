...
class QueueModel extends ModelBase
    ...
    function addTask($task)
        ...
        switch ($task->taskType)
        {
            ...
            case QS_PLUS9:
                {
                $justUpdate = isset($this->tasksInQueue[QS_PLUS9]);
                break;
                }
            case QS_PLUS10:
                {
                $justUpdate = isset($this->tasksInQueue[QS_PLUS10]);
                break;
                }
            case QS_PLUS11:
                {
                $justUpdate = isset($this->tasksInQueue[QS_PLUS11]);
                break;
                }
            case QS_PLUS12:
                {
                $justUpdate = isset($this->tasksInQueue[QS_PLUS12]);
                break;
                }
            case QS_PLUS13:
                {
                $justUpdate = isset($this->tasksInQueue[QS_PLUS13]);
                break;
                }
            case QS_PLUS14:
                {
                $justUpdate = isset($this->tasksInQueue[QS_PLUS14]);
                break;
                }
            case QS_PLUS15:
                {
                $justUpdate = isset($this->tasksInQueue[QS_PLUS15]);
                }
        }
        $qid = 0;
        if ($justUpdate)
 ...
 {
 if ((((isset ($this->tasksInQueue[$task->taskType]) AND isset ($this->tasksInQueue[$task->taskType][$task->buildingId])) AND $task->buildingId != 29) AND $task->buildingId != 30))
 {
 ...
 $extraTime = $_qt['remainingSeconds'];
            ...
            case QS_CREATEVILLAGE:
                ...
                if ($task->tag['troops'] != NULL)
                    ...
                    $this->_updateVillage(FALSE, FALSE, $this->_getNewTroops($task->tag['troops']));
                        ...
                        {
                        // fix crop +++
						$parentdata = $this->provider->fetchRow('SELECT v.parent_id, v.is_oasis FROM p_villages v WHERE v.id=%s', array(
                            $task->villageId
                        ));
                       $parentdataa = $this->provider->fetchRow('SELECT v.parent_id, v.is_oasis FROM p_villages v WHERE v.id=%s', array(
                            $task->toVillageId
                        ));
						if(!$parentdataa['is_oasis']) {
    ...
    function cancelTask($playerId, $taskId)
        ...
        $row          = $this->provider->fetchRow('SELECT 
                                ...
                                UNIX_TIMESTAMP(q.end_date) - UNIX_TIMESTAMP() remainingSeconds
                        FROM p_queue q 
                        WHERE 
        ...
        switch ($taskType)
        {
            ...
            case QS_BUILD_CREATEUPGRADE:
                ...
                foreach ($nextResources as $k=>$v) 
                   { 
                    $this->page->resources[$k]['current_value'] += ($v - $curResources[$k])-(($v - $curResources[$k])*20/100);   
                    }
