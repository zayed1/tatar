...
function showManual(b, c) {
    ...
    if (p != null) p.innerHTML = '<div id="_pwin" class="popup3" style="right:180px;top:99px;"><div id="drag" onmousedown="dragStart(event, \'_pwin\')"></div><a href="#" onClick="hideManual(); return false;"><img src="core-f/style-f/default/img/un/x.gif" border="1" class="popup4" alt="Move"></a><iframe frameborder="0" id="Frame" src="help.php?c=' + b + "&id=" + c + '" width="412" height="440" border="0"></iframe></div>';
    return false
}
function hideManual() {
	p = document.getElementById("ce");
...
function renderMap(obj, largeMap) {
	...
	var url = "karte.php?id=" + id + (largeMap? '&l' : '');
	if( xmlHttp == null ) {
		window.location = url;
		mreq = true;
		return false;
			...
			if (xmlHttp.responseText.length > 0) {
				...
				for (var i = 0, count =_mp["mtx"].length; i < count; i++) {
					...
					for (var j = 0, count1 = _mpa.length; j < count1; j++) {
						...
						ea.setAttribute ( "href", "dorf3.php?id=" + _mpb[0] );
...
function slm () {
	var w = window.open("karte.php?l&id=" + _mp["mtx"][3][3][0] ,"map","top=100,left=25,width=1007,height=585");
	w.focus();
	return false;
}
...
function goto_guide (value) {
	...
	if (p!=null) {
		if(!_vflag) {
			p.innerHTML = '<div id="_pwin" class="popup3 quest"><div id="drag" onmousedown="dragStart(event, \'_pwin\')"></div><a href="#" onClick="showTask();return false;"><img src="core-f/style-f/x.gif" border="1" class="popup4" alt="Move"></a><img src="core-f/style-f/default/plus/loading.gif" width="48" height="48" alt="loading"></div>';
		}
			...
			if (xmlHttp.readyState == 4) {
				if (xmlHttp.status == 200 && _flag == -1 && !_vflag) {
					if (xmlHttp.responseText != '') {
						p.innerHTML = '<div id="_pwin" class="popup3 quest"><div id="drag" onmousedown="dragStart(event, \'_pwin\')"></div><a href="#" onClick="showTask();return false;"><img src="core-f/style-f/x.gif" border="1" class="popup4" alt="Move"></a>' + xmlHttp.responseText + '</div>';
						init();
					}
...
jQuery(document).ready(function($) {
	$('img.linksSwitch').click(function(){
		$('#linkstbody').toggleClass('hide');

		if($('#linkstbody').hasClass('hide')){
	...
	$('img.villagesSwitch').click(function(){
		$('#villagestbody').toggleClass('hide');

		if($('#villagestbody').hasClass('hide')){
			$(this).removeClass('opened');
			$(this).addClass('closed');
