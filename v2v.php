...
$this->errorTable = 'كوكيل غير مصرح لك بالهجوم';
return null;
}
if( $this->player->isAgent == 1 AND substr($this->player->actions, 1, 1) == 0 AND $_POST['c'] == 2) {
$this->errorTable = 'كوكيل غير مصرح لك بالتعزيز';
return null;
}
...
$this->errorTable = "انت لازلت تحت الحمايه";
return null;
}
}
$m = new WarModel ();
...
$this->errorTableSend = "<span class='error'><b>انت تهاجم عضو في تحالفك  !!</b><p>";
}
}
...
$war1 = $war11 + $war111;
    if ($war1 >= 100){ // عدد الهجمات الخارجة
$this->redirect ('build?id=39');
}else 
