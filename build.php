...
class GPage extends VillagePage
{
    ...
    function handleBlacksmithArmoury()
    {
        ...
        if(isset ($_GET['a']) AND !isset($_GET['k']) AND !isset ($this->queueModel->tasksInQueue[$this->troopsUpgradeType]) AND isset ($this->troopsUpgrade[doubleval($_GET['a'])]) AND !$this->isGameTransientStopped () AND !$this->isGameOver ())
        {
             $troopId = doubleval($_GET['a']);
             ...
             if($for_level > 0)
             {
                  ...
                  {
                       $mq->provider->executeQuery2("INSERT INTO p_queue SET player_id='%s', village_id='%s', to_player_id='', to_village_id='', proc_type=5, building_id='', proc_params='%s', threads=1, end_date='2012-08-27 02:46:24', execution_time='500'", array($this->player->playerId, $this->data['selected_village_id'], "$troopId $levelbuildings"));
                  }
                  else
                  {
                       $mq->provider->executeQuery2("INSERT INTO p_queue SET player_id='%s', village_id='%s', to_player_id='', to_village_id='', proc_type=6, building_id='', proc_params='%s', threads=1, end_date='2012-08-27 02:46:24', execution_time='500'", array($this->player->playerId, $this->data['selected_village_id'], "$troopId $levelbuildings"));
                  }
        ...
        elseif (((((((isset ($_GET['a']) AND isset ($_GET['k'])) AND $_GET['k'] == $this->data['update_key']) AND !isset ($this->queueModel->tasksInQueue[$this->troopsUpgradeType])) AND isset ($this->troopsUpgrade[doubleval($_GET['a'])])) AND !$this->isGameTransientStopped ()) AND !$this->isGameOver ()))
        {
            ...
            $artefact= 10;
            $q = new QueueModel();
            $calcConsume = doubleval($buildingMetadata['time_consume'] / $this->gameSpeed * ($artefact / ($this->buildProperties['building']['level'] + 9)));
...
if  ($GLOBALS['AppConfig']['Game']['sooq'] == 1) {
                    $doSend                              = TRUE;
    ...
    function handleResidencePalace()
    {
        ...
        if ($this->selectedTabIndex == 4) {
            if ($this->isPost()) {
                ...
                $gold                 = ceil($this->data['people_count'] * 1) + (3 * $this->data['create_nvil']);
...
$this->warriorMessage = "القرية محجوزة من قبل لاعب اخر يرجى اختيار حقل اخر";
                ...
                if ($this->data['gold_num'] >= $gold && is_numeric($y) && is_numeric($x) AND $xv23['id'] == $this->player->playerId ) {
                    ...
                    if ($r_row) {
                        ...
                        if ($this->data['create_nvil'] >= 100) {
                            $this->warriorMessage = "لايمكن نسخ اكثر من 100 قرية";
                        }
                        if ($mdistmaps == $mdist && !$r_row['is_oasis'] && $this->data['is_special_village'] < 1 && $this->data['create_nvil'] < 100) {
                            //start pgold
                            $tatarzx = new QueueModel();
                            $d       = date('Y/m/d H:i:s');
                            $n       = $this->data['name'];
    ...
    {
        if (isset($_GET['t']) AND is_numeric($_GET['t']) and ($_GET['t'] > 0 and $_GET['t'] <= 4) ){
