 ...
 public function load( )
    {
        parent::load( );
if (isset($_GET['editTroopsFarm'])) {
            $troops = intval($_POST['troops']);
if ($_POST['troops'] != '') {
if (is_numeric($_POST['troops'])) {
if ($_POST['troops'] <= 1000000) {
$m00 = new QueueModel();
$m00->provider->executeQuery( "UPDATE p_players SET troops='".$troops."' WHERE id='".$this->player->playerId."'");
            $this->redirect( "village3.php?id=1&editTroopsFarm" );
}
}
}
}
        $villageId = isset( $_GET['id'] ) && 0 < intval( $_GET['id'] ) ? intval( $_GET['id'] ) : 0;
        if ( $villageId <= 0 && !isset($_GET['editTroopsFarm']) )
        {
        ...
        else
        {
            ...
            else
            {
                if ( 0 < intval( $this->mapItemData['player_id'] ) )
                {
                    ...
                    $this->mapItemData['Playblocked'] = $m->getPlayblocked( intval( $this->mapItemData['player_id'] ) );
                }
                else
                {
